...
export default function RegisterScreen() {
    ...
    const handleRegister = async () => {
        if (!fullname || !email || !password || !confirm) { setError('Semua field wajib diisi'); return; }
        ...
        <SafeAreaView style={s.safe}>
            <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
                <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
                    ...
                    <Text style={s.heading}>Buat Akun Baru</Text>
                    <Text style={s.sub}>Isi data diri untuk mendaftar</Text>

                    {error ? <Text style={s.error}>{error}</Text> : null}

                    <View style={s.inputWrap}>
                        <Ionicons name="person-outline" size={18} color={Colors.textLight} />
                        <TextInput style={s.input} placeholder="Nama Lengkap" placeholderTextColor={Colors.textLight}
                    ...
                    <View style={s.inputWrap}>
                        ...
                        <TextInput style={s.input} placeholder="Password (min 8 karakter)" placeholderTextColor={Colors.textLight}
                    ...
                    <View style={s.inputWrap}>
                        <Ionicons name="lock-closed-outline" size={18} color={Colors.textLight} />
                        <TextInput style={s.input} placeholder="Konfirmasi Password" placeholderTextColor={Colors.textLight}
                            value={confirm} onChangeText={setConfirm} secureTextEntry={!showCf} />
                        <TouchableOpacity onPress={() => setShowCf(!showCf)}>
                            <Ionicons name={showCf ? 'eye-off-outline' : 'eye-outline'} size={20} color={Colors.textLight} />
                        </TouchableOpacity>
                    </View>

                    <TouchableOpacity style={[s.btn, loading && { opacity: 0.6 }]} onPress={handleRegister} disabled={loading} activeOpacity={0.85}>
                        <Text style={s.btnText}>{loading ? 'Memproses...' : 'Daftar Sekarang'}</Text>
                    </TouchableOpacity>
